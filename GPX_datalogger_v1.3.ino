/*
    GPX datalogger v1.3   27-10-2018 by DoppioZero
    basic gpx datalogger

    -v1.3 added purple color when signal is lost during logging;
          minor bug fixes on RGB led
    -v1.2 added RGB led
...
void blinkLED() {
  ...
  if (validDATA == false && startLog == false) { // red blink until fixing is complete and all data is available
  ...
  if (startLog == true && validDATA == true) { // blue blink during logging
  ...
  if (startLog == true && validDATA == false) { // if signal is lost during logging blink purple
    if (millis() - previousMillis > 1000 && SDLEDstatus == 0) {
      digitalWrite(SDLED, HIGH);
      digitalWrite(FIXINGLED, HIGH);
      digitalWrite(DATALED, LOW);
